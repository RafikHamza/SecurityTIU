...
function openDatabase() {
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            // TODO: Add indexes if you need to query by other properties later
            // Example: store.createIndex('quizType', 'quizScores.quizType', { unique: false });
        };
...
async function loadProgressFromDB(pseudoid) {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
            ...
            // Don't reject, just resolve with null or handle the error appropriately
            resolve(null);
        };
    });
}
...
// --- Global State (to hold current pseudoid and progress) ---
let currentPseudoid = null;
let currentUserProgress = { completedModules: [], quizScores: {} }; // Initialize empty progress

// Function to set the current pseudoid and load/initialize progress
async function setCurrentPseudoid(pseudoid) {
    currentPseudoid = pseudoid;
    console.log('Current Pseudoid set:', currentPseudoid);

    if (pseudoid) {
        sessionStorage.setItem('currentPseudoid', pseudoid);
        // Load progress for this pseudoid
        const loadedProgress = await loadProgressFromDB(pseudoid);
        if (loadedProgress) {
            currentUserProgress = loadedProgress;
            console.log('Loaded progress for current pseudoid:', currentUserProgress);
        } else {
        ...
        currentUserProgress = { completedModules: [], quizScores: {} }; // Clear progress if pseudoid is unset
        console.log('Current Pseudoid unset. Progress cleared.');
    }
...
function getCurrentPseudoid() {
    return currentPseudoid; // Return the globally tracked pseudoid
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        console.log('Database ready.');
         ...
         if (savedPseudoid) {
             // Use the async setCurrentPseudoid to load progress
             await setCurrentPseudoid(savedPseudoid);
             console.log('Loaded pseudoid and progress from session storage.');
            ...
            if (typeof moduleData.init === 'function') {
                ...
                if (moduleName === 'profile') {
                    moduleData.init(contentArea, {
                        ...
                        getCurrentUserProgress: getCurrentUserProgress // Pass getter for current progress
                    });
                } else {
