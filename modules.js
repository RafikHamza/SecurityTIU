...
export const modules = {
    ...
    profile: {
        ...
        init: function(contentElement, dbFunctions) {
             ...
             // Load the currently set pseudoid and its progress when the profile page loads
             const initialPseudoid = dbFunctions.getCurrentPseudoid();
             if (initialPseudoid) {
                 pseudoidInput.value = initialPseudoid; // Populate input field
                 dbFunctions.loadProgressFromDB(initialPseudoid).then(progress => {
                      displayProgress(progress);
                      updatePseudoidDisplay(initialPseudoid);
                 }).catch(error => {
                     console.error('Error loading initial profile progress:', error);
                     displayProgress(null);
                     updatePseudoidDisplay('Error loading progress');
                 });
             } else {
                 displayProgress(null);
